...
厂商下载地址分为固定链接和带签名的短期动态链接。Tapp 会大小写不敏感地检查 URL 中的 `Expires`、`Signature`、`sign`、`auth_key` 及 AWS/Google/OSS 等明确签名参数，并从可识别的参数推导 `expiresAt`；无签名的 ZIP/TGZ 才可由自有 API 长期缓存。带签名结果过期后会重新解析。服务限流、令牌过期或厂商接口异常时，页面会保留目录原始地址。下载和刷机均有风险，请核对型号、地区、MD5 与包类型。
...
- 根据 SmartTool 反编译逻辑识别魅族 `auth_key` 动态签名，并支持常见云存储签名参数
- 从可识别的签名参数推导过期时间，防止自有 API 中误标的短期链接直接短路外部解析

### v1.3.2 (2026-08-18)

- 对目录自身提供的 ZIP 直链使用相同的固定/动态签名判定，不再遗漏带 `auth_key` 或云存储签名的直链
